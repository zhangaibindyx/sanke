...
import com.zab.sanke.R.styleable;
...
/**
 * Created by jinkia on 2016/9/30. 摇杆控件
 */
@SuppressLint("NewApi")
public class RockerView extends View {
	...
	private Direction tempDirection = Direction.CENTER;
	...
	protected void onDraw(Canvas canvas) {
		...
		// 可移动区域的半径
		mAreaRadius = (measuredWidth <= measuredHeight) ? (int) (cx / (mRockerScale + 1))
				: (int) (cy / (mRockerScale + 1));
		...
		if (AREA_BACKGROUND_MODE_PIC == mAreaBackgroundMode || AREA_BACKGROUND_MODE_XML == mAreaBackgroundMode) {
			...
			Rect dst = new Rect(mCenterPoint.x - mAreaRadius, mCenterPoint.y - mAreaRadius,
		...
		// 画摇杆
		if (ROCKER_BACKGROUND_MODE_PIC == mRockerBackgroundMode
				|| ROCKER_BACKGROUND_MODE_XML == mRockerBackgroundMode) {
			// 图片
			Rect src = new Rect(0, 0, mRockerBitmap.getWidth(), mRockerBitmap.getHeight());
			Rect dst = new Rect(mRockerPosition.x - mRockerRadius, mRockerPosition.y - mRockerRadius,
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:// 移动
			...
			mRockerPosition = getRockerPositionPoint(mCenterPoint, new Point((int) moveX, (int) moveY),
					mAreaRadius + mRockerRadius, mRockerRadius);
		...
		case MotionEvent.ACTION_CANCEL:// 移出区域
			...
			if (mOnShakeListener != null) {
				mOnShakeListener.direction(Direction.CENTER);
			}
	...
	private Point getRockerPositionPoint(Point centerPoint, Point touchPoint, float regionRadius, float rockerRadius) {
		...
		if (lenXY + rockerRadius <= regionRadius) { // 触摸位置在可活动范围内
			...
			callBack(angle, (int) Math.sqrt((showPointX - centerPoint.x) * (showPointX - centerPoint.x)
					+ (showPointY - centerPoint.y) * (showPointY - centerPoint.y)));
		...
		if (null != mOnShakeListener) {
			if (CallBackMode.CALL_BACK_MODE_MOVE == mCallBackMode) {
				...
				case DIRECTION_2_HORIZONTAL:// 左右方向
					if (ANGLE_0 <= angle && ANGLE_HORIZONTAL_2D_OF_0P > angle
							|| ANGLE_HORIZONTAL_2D_OF_1P <= angle && ANGLE_360 > angle) {
						// 右
						mOnShakeListener.direction(Direction.RIGHT);
					} else if (ANGLE_HORIZONTAL_2D_OF_0P <= angle && ANGLE_HORIZONTAL_2D_OF_1P > angle) {
						// 左
						mOnShakeListener.direction(Direction.LEFT);
					}
					break;
				case DIRECTION_2_VERTICAL:// 上下方向
					if (ANGLE_VERTICAL_2D_OF_0P <= angle && ANGLE_VERTICAL_2D_OF_1P > angle) {
						// 下
						mOnShakeListener.direction(Direction.DOWN);
					} else if (ANGLE_VERTICAL_2D_OF_1P <= angle && ANGLE_360 > angle) {
						// 上
						mOnShakeListener.direction(Direction.UP);
					}
					break;
				case DIRECTION_4_ROTATE_0:// 四个方向
					if (ANGLE_4D_OF_0P <= angle && ANGLE_4D_OF_1P > angle) {
						// 右下
						mOnShakeListener.direction(Direction.RIGHT_DOWN);
					} else if (ANGLE_4D_OF_1P <= angle && ANGLE_4D_OF_2P > angle) {
						// 左下
						mOnShakeListener.direction(Direction.LEFT_DOWN);
					} else if (ANGLE_4D_OF_2P <= angle && ANGLE_4D_OF_3P > angle) {
						// 左上
						mOnShakeListener.direction(Direction.LEFT_UP);
					} else if (ANGLE_4D_OF_3P <= angle && ANGLE_360 > angle) {
						// 右上
						mOnShakeListener.direction(Direction.RIGHT_UP);
					}
					break;
				case DIRECTION_4_ROTATE_45:// 四个方向 旋转45度
					if (ANGLE_0 <= angle && ANGLE_ROTATE45_4D_OF_0P > angle
							|| ANGLE_ROTATE45_4D_OF_3P <= angle && ANGLE_360 > angle) {
						// 右
						mOnShakeListener.direction(Direction.RIGHT);
				...
				case DIRECTION_2_VERTICAL:// 上下方向
					if (ANGLE_VERTICAL_2D_OF_0P <= angle && ANGLE_VERTICAL_2D_OF_1P > angle
						...
						tempDirection = Direction.DOWN;
						mOnShakeListener.direction(Direction.DOWN);
					} else if (ANGLE_VERTICAL_2D_OF_1P <= angle && ANGLE_360 > angle && tempDirection != Direction.UP) {
						// 上
					...
					break;
				case DIRECTION_4_ROTATE_0:// 四个方向
					if (ANGLE_4D_OF_0P <= angle && ANGLE_4D_OF_1P > angle && tempDirection != Direction.RIGHT_DOWN) {
						// 右下
						tempDirection = Direction.RIGHT_DOWN;
						mOnShakeListener.direction(Direction.RIGHT_DOWN);
					} else if (ANGLE_4D_OF_1P <= angle && ANGLE_4D_OF_2P > angle
							&& tempDirection != Direction.LEFT_DOWN) {
						// 左下
						tempDirection = Direction.LEFT_DOWN;
						mOnShakeListener.direction(Direction.LEFT_DOWN);
					} else if (ANGLE_4D_OF_2P <= angle && ANGLE_4D_OF_3P > angle
							&& tempDirection != Direction.LEFT_UP) {
						// 左上
						tempDirection = Direction.LEFT_UP;
						mOnShakeListener.direction(Direction.LEFT_UP);
					} else if (ANGLE_4D_OF_3P <= angle && ANGLE_360 > angle && tempDirection != Direction.RIGHT_UP) {
						// 右上
						tempDirection = Direction.RIGHT_UP;
						mOnShakeListener.direction(Direction.RIGHT_UP);
					}
					break;
				case DIRECTION_4_ROTATE_45:// 四个方向 旋转45度
				...
				case DIRECTION_8:// 八个方向
					if ((ANGLE_0 <= angle && ANGLE_8D_OF_0P > angle || ANGLE_8D_OF_7P <= angle && ANGLE_360 > angle)
						...
						tempDirection = Direction.RIGHT_DOWN;
						mOnShakeListener.direction(Direction.RIGHT_DOWN);
					} else if (ANGLE_8D_OF_1P <= angle && ANGLE_8D_OF_2P > angle && tempDirection != Direction.DOWN) {
						// 下
						tempDirection = Direction.DOWN;
						mOnShakeListener.direction(Direction.DOWN);
					} else if (ANGLE_8D_OF_2P <= angle && ANGLE_8D_OF_3P > angle
						...
						tempDirection = Direction.LEFT_DOWN;
						mOnShakeListener.direction(Direction.LEFT_DOWN);
					} else if (ANGLE_8D_OF_3P <= angle && ANGLE_8D_OF_4P > angle && tempDirection != Direction.LEFT) {
						// 左
						tempDirection = Direction.LEFT;
						mOnShakeListener.direction(Direction.LEFT);
					} else if (ANGLE_8D_OF_4P <= angle && ANGLE_8D_OF_5P > angle
						...
						tempDirection = Direction.LEFT_UP;
						mOnShakeListener.direction(Direction.LEFT_UP);
					} else if (ANGLE_8D_OF_5P <= angle && ANGLE_8D_OF_6P > angle && tempDirection != Direction.UP) {
						// 上
						tempDirection = Direction.UP;
						mOnShakeListener.direction(Direction.UP);
					} else if (ANGLE_8D_OF_6P <= angle && ANGLE_8D_OF_7P > angle
	...
	public void setOnShakeListener(OnShakeListener listener) {
		...
		mDirectionMode = DirectionMode.DIRECTION_8;
