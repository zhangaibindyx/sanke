...
/**
 * 把屏幕划分成一个二维的矩阵,用int[][]来记录每一个方格的状态 计算横,纵向下图形(方格)的个数 计算横纵向的偏移量,用来绘制每个方格时定位
 * 初始化二维数组
 * */
@SuppressLint({ "ClickableViewAccessibility", "DrawAllocation", "Recycle",
"HandlerLeak" })
public class PkGame extends View {
	private int size = 30; // 图片的大小
	private int xcount = 70; // 定义图片的x上需要放的数量
	private int ycount = 50; // 定义图片的y上需要放的数量
	...
	// 好友
	private ArrayList<Coordinate> snakeListFriend = new ArrayList<Coordinate>();
	 ...
	 */
	public void up() {
		if (direction != UP && direction != PkGame.DOWN)
			nextdirection = UP;
	}
	 ...
	 */
	public void down() {
		if (direction != DOWN & direction != PkGame.UP)
			nextdirection = DOWN;
	}
	 ...
	 */
	public void left() {
		if (direction != LEFT && direction != PkGame.RIGHT)
			nextdirection = LEFT;
	}
	 ...
	 */
	public void right() {
		if (direction != RIGHT && direction != PkGame.LEFT)
			nextdirection = RIGHT;
	}

	// 复写了2个参数的构造方法
	public PkGame(Context context, AttributeSet attrs) {
		super(context, attrs);
		isMachine = false;
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < xcount; i++) {
			for (int j = 0; j < ycount; j++) {
				...
				{
					canvas.drawBitmap(pics[map[i][j]], xoffset + i * size,
							yoffset + j * size, paint);
				}
			}
		}
	}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if(isFirnd){
			xcount = (int) Math.floor(1280 / size); // x方向需要的图片数量,向下取整
			ycount = (int) Math.floor(720 / size);
	...
	public void loadPic(int key, Drawable drawable) {
		/*
		 * 1、Drawable就是一个可画的对象，其可能是一张位图（BitmapDrawable），也可能是一个图形（ShapeDrawable），
		 * 还有可能是一个图层（LayerDrawable），我们根据画图的需求，创建相应的可画对象 2、Canvas画布，绘图的目的区域，用于绘图
		 * 3、Bitmap位图，用于图的处理 4、Matrix矩阵
		 */
	...
	private void initFriendScank() {
		snakeListFriend.add(new Coordinate(xcount - 5, 7));
		snakeListFriend.add(new Coordinate(xcount - 4, 7));
		snakeListFriend.add(new Coordinate(xcount - 3, 7));
		snakeListFriend.add(new Coordinate(xcount - 2, 7));
	}

	// 组建墙
	/*
	 * 完成墙绘制之前需要加载游戏中用的图片,红色(蛇头),黄色(蛇身),绿色(墙,苹果) 为地图位置赋值
	 */
	 ...
	 * */
	private boolean isFirnd = false;

	public void setFrindPK(boolean isFrind) {
		this.isFirnd = isFrind;
	}
	...
	private boolean friendinvincible = false;

	public void setFriendInvincible() {
		friendinvincible = true;
		new Thread() {
			public void run() {
				SystemClock.sleep(3000);
				friendinvincible = false;
			};
		}.start();
	}
	...
	private int nextFrindDirection = LEFT;
	private int frindDirection = LEFT;

	public void setFriendDirection(int dir) {
		nextFrindDirection = dir;
	}

	public void upFriendSnke() {
		Coordinate header = snakeListFriend.get(0);
		Coordinate newHeader = new Coordinate(1, 1); // 新的蛇头
		frindDirection = nextFrindDirection;
		switch (frindDirection) {
		case RIGHT:
		...
		if (!friendinvincible) {
			// 是否撞墙
			out:for (int i = 0; i < wallMap.length; i++) {
				for (int j = 0; j < wallMap[i].length; j++) {
					if (wallMap[i][j] == true) {

						if (newHeader.x == i && newHeader.y == j) {
							snakeListFriend.clear();
							newHeader.x=xcount - 6;
							newHeader.y= 7;
							initFriendScank();
							break out;
						}
		...
		// 检测蛇是否吃到自己
		for (Coordinate c : snakeListFriend) {
		...
		// 蛇头吃到苹果
		if (newHeader.x == appleList.get(0).x
				&& newHeader.y == appleList.get(0).y) {
		...
		snakeListFriend.add(0, newHeader);
		if (!growSnake) // 如果没吃到果子则去掉最后一节蛇,吃到了则不清理最后一节
		{
			// 将原来的蛇的尾部除掉
			snakeListFriend.remove(snakeListFriend.size() - 1);
		}

		// index是蛇的具体哪一节
		int index = 0;
		for (Coordinate c : snakeListFriend) // 迭代取出ArrayList<Coordinate>中的数据
		{
			if (index == 0) // 蛇头,绘制黄色
			{
				setTile(TOU_STAT, c.x, c.y);
			} else if (index != 0 && index != snakeListFriend.size() - 1) // 蛇身,绘制红色
			{
	...
	public void updateSnake() {
		...
		if (newHeader.x < 0 || newHeader.x > xcount - 1 || newHeader.y < 0
				|| newHeader.y > ycount - 1) {
			setMode(LOSE);
			return;
		}
	...
	/**
	 * 主要用于游戏结束后继续游戏采用的初始化动作。 注意此时的分数是接着上一次结束时的分数 注意此时的速度是接着上一次结束时的速度
	...
	public interface GameOverListener {
		/**
		 * 当此方法被执行时，代表游戏已经结束，血量为0。 此方法处理游戏退出和复活时使用。
		 */
		void GameOver();
	}
	...
	// 给苹果绘图
	private void updateApple() { // 金苹果时，开启抽奖模式
		if (isGoldPg) {
